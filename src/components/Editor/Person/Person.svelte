<script>
  ...
  import { rssData, editingEpisode } from "../../../../stores/editor";
...
{#if $rssData && $editingEpisode["podcast:person"]}
  <section class="tab-zone" bind:clientWidth={paneWidth}>
    <button
      class:active={type === "episode"}
      on:click={() => (type = "episode")}
    >
      <p>Episode</p>
    </button>
    <button
      class:active={type === "podcast"}
      on:click={() => (type = "podcast")}
    >
      <p>Podcast</p>
    </button>
  ...
  <div class="extra-wide" style={`right:${type === "episode" ? "100%" : "0"}`}>
    <Podcast bind:data={$rssData["podcast:person"]} type={"podcast"} />
    <Podcast bind:data={$editingEpisode["podcast:person"]} type={"episode"} />
  </div>
{/if}
